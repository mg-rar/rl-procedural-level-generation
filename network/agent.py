...
class D3QNAgent:
    ...
    def save(self, path):
        torch.save(self.q_network.state_dict(), f'{path}.q')
        torch.save(self.target_network.state_dict(), f'{path}.target')
        self.memory.save(f'{path}.replay')

    def load(self, path):
        self.q_network.load_state_dict(torch.load(f'{path}.q'))
        self.target_network.load_state_dict(torch.load(f'{path}.target'))
        self.memory.load(f'{path}.replay')
